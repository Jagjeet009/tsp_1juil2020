...
class Android extends CI_Controller {
	...
	function getFromToListIds($parameters,$all_element_ids_linewise,$all_code_name_ids_linewise){
			...
			foreach($this->function_list as $fl_k=>$fl_v){		//search for functions in line
				if(strstr($statement,$fl_k)){
					...
					if($pre_code_new['function']=="skip" || $pre_code_new['function']=="doHide" || $pre_code_new['function']=="doShow" || $pre_code_new['function']=="doJumpForward"){
						$pre_code_new['parameters']=$this->getFromToListIds($pre_code_new['parameters'],$all_element_ids_linewise,$all_code_name_ids_linewise);
							...
							for($l=0;$l<sizeof($pre_code[$i]);$l++){
								if($pre_code[$i][$l]!=''){
									...
									if($found==true && $found_function!=''){
										...
										$pre_code[$i][$l]=array('constant'=>$this->constantStatementsToJson($pre_code[$i][$l]));
									}	
					...
					for($l=0;$l<sizeof($pre_code);$l++){
						if($pre_code[$l]!=''){
							...
							if($found==true && $found_function!=''){
								...
								$pre_code[$l]=array('constant'=>$this->constantStatementsToJson($pre_code[$l]));
							}	
							...
							for($l=0;$l<sizeof($post_code[$i]);$l++){
								if($post_code[$i][$l]!=''){
									...
									if($found==true && $found_function!=''){
										...
										$post_code[$i][$l]=array('constant'=>$this->constantStatementsToJson($post_code[$i][$l]));
									}	
					...
					for($l=0;$l<sizeof($post_code);$l++){
						if($post_code[$l]!=''){
							...
							if($found==true && $found_function!=''){
								...
								$post_code[$l]=array('constant'=>$this->constantStatementsToJson($post_code[$l]));
							}	

...
$('input[name="graph-view"],input[name="chart-view"],input[name="map-view"],select[name="chart-draw-view"]').on('change',function(){
	generateGraph();
});
...
function generateGraph(){
	...
	if(graphType=="frequency"){
		...
		if(rows_arr_l==0 && columns_arr_l==0){
			...
			$('[name="analytics_post_data"]').val('{"rows":'+rows_arr+',"columns":'+columns_arr+',"indicators":'+indicators+',"indicators_dataid":'+indicators_dataid+'}');
			
			//calculate data for regression in json
			$.post("<?php echo base_url(); ?>survey/analytics/calculateregression/"+survey_id,{"rows":rows_arr,"columns":columns_arr,"indicators":indicators,"indicators_dataid":indicators_dataid}).then( function(result1) {
				//console.log(result1);
...
$(document).on("keyup", ".analytics-search-indicator-panel input[name='search_filter']",function(){
	...
	var ids_to_show=[];
	$('#legend-lable-list').find("li:containsIN("+search_text+")").each(function(){
		var li=$(this);
		var li_no=li.data('serial');

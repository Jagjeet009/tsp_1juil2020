...
class Survey_model extends CI_Model{
    ...
    public function store_survey($data){
		$insert = $this->db->insert('survey', $data);
	    return $this->db->insert_id();
	}
    public function store_survey_section($data){
    ...
    public function publish_survey($id, $data=''){
		$data=array(
			'publish_date' => time()
		);
		$this->db->where('id', $id);
	...
	public function delete_survey($title_url){
		$this->db->where('title_url', $title_url);
	...
	public function delete_section_by_title_url($title_url){
		$this->db->where('survey_id', $title_url);
		$this->db->delete('survey_data'); 
		$this->db->where('title_url', $title_url);
		$this->db->delete('survey_section'); 
	}
    public function activate_survey($title_url, $data=array('status' => '1')){
		$this->db->where('title_url', $title_url);
		$this->db->update('survey', $data);
		$report = array();
		//$report['error'] = $this->db->_error_number();
    ...
    public function deactivate_survey($title_url, $data=array('status' => '0')){
		$this->db->where('title_url', $title_url);
		$this->db->update('survey', $data);
		$report = array();
		//$report['error'] = $this->db->_error_number();
    ...
    public function count_surveys($search_string=null, $order=null){
		$this->db->select('*');
		$this->db->from('survey');
		if($search_string){
	...
	public function delete_partial_by_ids($survey_case_id,$survey_id,$section_id){
		$this->db->where('survey_id', $survey_id);
		$this->db->where('section_id', $section_id);
		$this->db->where('survey_case_id', $survey_case_id);
		$this->db->delete('survey_partial_values'); 
	}
	public function delete_partial_by_id($survey_case_id,$survey_id){
		$this->db->where('survey_id', $survey_id);
		$this->db->where('survey_case_id', $survey_case_id);
		$this->db->delete('survey_partial_values'); 
		//echo $this->db->last_query();
	...
	public function survey_indicators_dataid_codenames($survey_title_url){
		...
		$code_names=array();
		$lengths=array();
		$indicators_type=array();
		$query=$this->db->query("select data_id,qtype,json_data,code_name,lengths from survey_data where survey_id in (select title_url from survey_section where survey_id = '$survey_id' ) ");
		if($query->num_rows()>0){
			...
			foreach($query as $q){
				...
				if(isset($q['lengths'])){
					$q['lengths']=(array) json_decode($q['lengths']);
					$lengths=array_merge($lengths,$q['lengths']);	
					//print_r($lengths);						
					foreach($indicators as $indi_k=>$indi_v){
						if(array_key_exists("format_answer_".strtolower($indi_k),$lengths)){
							if($lengths["format_answer_".strtolower($indi_k)]=="F"){
								$indicators_type[$indi_k]='N';
							}else{
								$indicators_type[$indi_k]='A';
							}
						}
					}
				}				
		...
		return array("indicators"=>$indicators,"indicators_dataid"=>$indicators_dataid,"code_names"=>$code_names,"indicators_type"=>$indicators_type);
	}

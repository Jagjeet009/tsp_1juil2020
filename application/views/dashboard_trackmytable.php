...
<div class="row small-cards">
	...
	<h6>
		<div class="dropdown survey-selector">
		  ...
		  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
		  	...
		  	<a class="dropdown-item <?php if($survey_id==$survey['title_url']){echo 'active';} ?>" href="<?php echo base_url()."dashboard/trackmytable/".$dashboard_url."/".$survey['title_url']; ?>"><?php echo $survey['title']; ?></a>
		  	<?php } ?>
		  </div>
		</div>		
	</h6>
	...
	if(isset($survey_id) && $survey_id!=''){
		$table_query=$this->db->query("select * from dashboard_analytics where survey_id = '".$survey_id."' and analytics_graph_url like '%table%' and analytic_or_track='1' ");
	}else{
		$table_query=$this->db->query("select * from dashboard_analytics where survey_id in ('".implode("','",$surveys_title_urls)."') and analytics_graph_url like '%table%' and analytic_or_track='1' ");
	}

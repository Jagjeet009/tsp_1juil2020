...
function convert_code_from_code2(code2form) {
     ...
     for (var i = 0; i < elements.length; i++) {
          ...
          element_html += '/*preproc*/' + '\r\n' + fieldset_preproc_function_list_html + '/*preproc*/' + '\r\n';
          ...
          if (fieldset_preproc_conditional_elseif.length > 0) {
               ...
               fieldset_preproc_conditional_elseif.find('fieldset.condition').find('fieldset.rule').each(function () {
                    var rule = $(this);
                    var co = rule.find('.no-class').find('.conditional-operators');
                    if (!co.hasClass('hidden')) { //it is visible only
                         fieldset_preproc_conditional_elseif_condition_text += ' ' + co.val() + ' ';
                    }
                    var vid = rule.find('.no-class').find('.variable_ids');
                    fieldset_preproc_conditional_elseif_condition_text += '' + vid.val();

                    var o = rule.find('.no-class').find('.operators');
                    fieldset_preproc_conditional_elseif_condition_text += '' + o.val();

                    var cv = rule.find('.no-class').find('.conditionValue ');
                    fieldset_preproc_conditional_elseif_condition_text += '' + cv.val();
               });
               fieldset_preproc_conditional_elseif_condition_text = 'else if(' + fieldset_preproc_conditional_elseif_condition_text + '){' + '\r\n';
               var fieldset_preproc_function_list_html = '';
               fieldset_preproc_conditional_elseif.find('fieldset.functions').find('>fieldset').each(function () {
                    var f = $(this);
                    var f_name = f.attr('class');
                    //fieldset_preproc_function_list_html += f_name + '\r\n';
               ...
               //console.log(fieldset_preproc_function_list_html);
               fieldset_preproc_conditional_elseif_condition_text += fieldset_preproc_function_list_html + '}';
          ...
          if (fieldset_postproc_conditional_elseif.length > 0) {
               ...
               fieldset_postproc_conditional_elseif.find('fieldset.condition').find('fieldset.rule').each(function () {
                    var rule = $(this);
                    var co = rule.find('.no-class').find('.conditional-operators');
                    if (!co.hasClass('hidden')) { //it is visible only
                         fieldset_postproc_conditional_elseif_condition_text += ' ' + co.val() + ' ';
                    }
                    var vid = rule.find('.no-class').find('.variable_ids');
                    fieldset_postproc_conditional_elseif_condition_text += '' + vid.val();

                    var o = rule.find('.no-class').find('.operators');
                    fieldset_postproc_conditional_elseif_condition_text += '' + o.val();

                    var cv = rule.find('.no-class').find('.conditionValue ');
                    fieldset_postproc_conditional_elseif_condition_text += '' + cv.val();
               });
               fieldset_postproc_conditional_elseif_condition_text = 'else if(' + fieldset_postproc_conditional_elseif_condition_text + '){' + '\r\n';
               var fieldset_postproc_function_list_html = '';
               fieldset_postproc_conditional_elseif.find('fieldset.functions').find('>fieldset').each(function () {
                    var f = $(this);
                    var f_name = f.attr('class');
                    //fieldset_postproc_function_list_html += f_name + '\r\n';
               ...
               //console.log(fieldset_postproc_function_list_html);
               fieldset_postproc_conditional_elseif_condition_text += fieldset_postproc_function_list_html + '}';
...
function convert_code_from_function(f){
     ...
     if(f_name=="getVal"){
          ...
          var element_ids=f.find('.element_ids').val();
          var variable_ids=f.find('.variable_ids').val();
          var languages=f.find('.languages_text').data('languages');
...
$(document).on('click', '.add-function', function () {
     ...
     var survey_id = $(this).closest('form').data('survey-id');

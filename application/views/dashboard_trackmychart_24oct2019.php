...
<div class="row small-cards">
	...
	if(isset($survey_id) && $survey_id!=''){
		$table_query=$this->db->query("select * from dashboard_analytics where survey_id = '".$survey_id."' and analytics_graph_url like '%chart%' and analytic_or_track='1' ");
	}else{
		$table_query=$this->db->query("select * from dashboard_analytics where survey_id in ('".implode("','",$surveys_title_urls)."') and analytics_graph_url like '%chart%' and analytic_or_track='1' ");
	}

...
$row_col_entries_percent=array();
$temp_r=array();
foreach($row_col_entries as $rce_k=>$rce_v){
	$temp_c=array();
	foreach($rce_v as $rce_v_k=>$rce_v_v){
		if(array_key_exists($rce_k,$rows_options_count)){
			if($rows_options_count[$rce_k]>1){
				$temp_c[$rce_v_k]=(($rce_v_v/$rows_options_count[$rce_k])*100);
				$temp_c[$rce_v_k]=(float) number_format($temp_c[$rce_v_k],1);
			}
		}
	}
	$temp_r[$rce_k]=$temp_c;
}
$row_col_entries_percent=$temp_r;

$d1=array();
$temp_arr=array();
array_push($temp_arr,array($data['rows'][0],$data['layer'][0],"Percentage"));
foreach($row_col_entries_percent as $rce_k=>$rce_v){
	foreach($rce_v as $rce_v_k=>$rce_v_v){
		$temp_row_arr=array();
	...
	function drawMainDashboard<?php echo $rand ?>() {
		...
		var area = new google.visualization.ChartWrapper({
		  ...
		  'options': {
			...
			'pieSliceText': 'percentage'		//value percentage
			//'pieHole':0.4,
		  },
		  'view': {'columns': [0, 2]}
		});		

...
$indicators_type=$sidc['indicators_type'];
//print_r($indicators_type);
    ...
    <div class="disp-table active clearfix">
        ...
        <div class="clearfix analytics-indicator-panel">
          ...
          <div>
            <ul id="legend-lable-list">
              <?php $serial=0;foreach($indicators as $indi_key => $indi_value){
              	$serial++;
              	if(!isset($indicators_type[$indi_key])){
              		$indicators_type[$indi_key]='';
              	}
              	 ?>
             <li data-serial="<?php echo $serial; ?>" title="<?php echo $indi_value; ?>"><?php echo "[".$indicators_type[$indi_key]."] ".substr($indi_value,0,15); ?></li>
             <?php } ?>
            </ul>
          </div>
        </div>
      ...
      <div class="sur-graph-type disp-table-cell">
        ...
        <form>
          ...
          <label>
            <input type="radio" name="graph-view" value="word-cloud" class="graph-view" /> Word Cloud 
          </label> 		  
          <div class="map-view">
			  <label>
...
$(document).on('DOMSubtreeModified', ".tmp_image_container", function() {
	console.log("tmp_image_container modified");
	var chart=$(this).closest('.columnchart_values');
	createImageFromDataurl(chart);
});
function createImageFromDataurl(chart){
	//console.log(chart);
	var tmp_img=chart.find('.tmp_img');
	var data_url=tmp_img.attr('src');
	var id=chart.attr('id');
	var container=$('#graph-container');
	if(id!=""){
		$.post("<?php echo base_url(); ?>survey/analytics/png2/"+id,{"data_url":data_url}).then(function(data){
			//container.find('.tmp_img').remove();
...
function generateGraph(){
	...
	if(rows_arr_l>0){
		$('#graph-container').html('<div class="loader-box"><img src="<?php echo base_url(); ?>theme/images/ajax-loader.gif" class="loader"></div>');
	}
	if(graphType=="frequency"){
		...
		$('#color_pallete').hide();
		$('#table_template').hide();
	}else if(graphType=="word-cloud"){
		if(rows_arr_l>0){
			$('[name="analytics_graph_url"]').val("survey/analytics/word_cloud/"+survey_id);
			$('[name="analytics_post_data"]').val('{"rows":'+rows_arr+',"indicators":'+indicators+',"indicators_dataid":'+indicators_dataid+'}');
			var post_data_json='';
			var jq_id='';
			var data_url='';
			post_data_json={"rows":rows_arr,"indicators":indicators,"indicators_dataid":indicators_dataid};		
			$.post("<?php echo base_url(); ?>survey/analytics/word_cloud/"+survey_id,post_data_json).then(function(data){
				data+='<div class="loader-box"><img src="<?php echo base_url()."theme/images/ajax-loader.gif"; ?>" class="loader" /></div>';
				//container.html(data);
				//console.log(data);
				$('#graph-container').html(data);
				jq_id=$('#graph-container').find('.columnchart_values').attr('id');
				var word_array=$('#graph-container').find('.columnchart_values').data('word_array');
				if(typeof word_array !== "undefined") {
					$('#'+jq_id).jQCloud(word_array);
				}			
			}).then(function(){
				setTimeout(function(){
					$('#graph-container').find('.loader-box').remove();	
					var jq_id=$('#graph-container').find('.columnchart_values')[0];	
					var jq_id_id=$('#graph-container').find('.columnchart_values').attr('id');
					var rect = jq_id.getBoundingClientRect();
					html2canvas(jq_id,{height:rect.height, width:rect.width}).then(function(canvas) {

						var data_url=canvas.toDataURL();
						var c=$('#graph-container').find('.tmp_image_container')[0];
						var img=document.createElement('img');
						img.src=canvas.toDataURL();
						img.className='tmp_img';
						c.appendChild(img);	
						c.setAttribute('data-image', data_url);	
					/*$.post("<?php echo base_url(); ?>survey/analytics/png2/"+jq_id_id,{"data_url":data_url}).then(function(data){
						//console.log(data);
						$('#graph-container').find('.loader-box').remove();	
						$('#graph-container').find('img').remove();
						var img=document.createElement('img');
						img.src='<?php echo base_url()."uploads/dashboard_chart_" ?>'+jq_id_id+'.png';
		...
		items: {
			"copy": {
				...
				callback: function(itemKey, opt, rootMenu, originalEvent){
					...
					if(gval=="chart"){
						var img=$('#graph-container .copy_img');

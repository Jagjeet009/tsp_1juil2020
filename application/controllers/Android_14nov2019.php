...
class Android extends CI_Controller {
	...
	function getIdsFromCountListIds($parameters,$all_element_ids_linewise,$all_code_name_ids_linewise,$function_name){
		...
		if(array_key_exists($parameters['id'],$all_code_name_ids_linewise)){
			$parameters['id']=$all_code_name_ids_linewise[$parameters['id']];
		}
		if(in_array($parameters['id'],$all_element_ids_linewise)){
			$from_index=array_search($parameters['id'],$all_element_ids_linewise);
		}
		if($function_name=="doColumnHide" || $function_name=="doColumnShow"){
			$temp=explode("_",$parameters['id']);
			$temp1=$temp[0]."_".$temp[1];
			
			$only_rc=str_replace($temp1."_","",$parameters['id']);
			if(substr($only_rc,0,1)=="0"){
				$only_rc="0".($only_rc+$parameters['Nos']);
			}else{
				$only_rc=($only_rc+$parameters['Nos']);
			}
			/*$new_all_element_ids=array();
			foreach($all_element_ids_linewise as $aeil){
				if(strpos($aeil,$temp1)>-1){
					array_push($new_all_element_ids,$aeil);
				}
			}*/
			$new_all_element_ids=array();
			$temp1dd="answer_dd_".str_replace("answer_","",$temp1);
			$temp1mm="answer_mm_".str_replace("answer_","",$temp1);
			$temp1yyyy="answer_yyyy_".str_replace("answer_","",$temp1);
			$temp1h="answer_h_".str_replace("answer_","",$temp1);
			$temp1m="answer_m_".str_replace("answer_","",$temp1);
			foreach($all_element_ids_linewise as $aeil){
				if(strpos($aeil,$temp1)>-1 || strpos($aeil,$temp1dd)>-1 || strpos($aeil,$temp1mm)>-1 || strpos($aeil,$temp1yyyy)>-1 || strpos($aeil,$temp1h)>-1 || strpos($aeil,$temp1m)>-1){
					array_push($new_all_element_ids,$aeil);
				}
			}			
			$ids=array();
			if(sizeof($new_all_element_ids)>0){
				$start_id=array_search($parameters['id'],$new_all_element_ids);
				$end_id=array_search($temp1."_".$only_rc,$new_all_element_ids);
				$ids=array_slice($new_all_element_ids,($start_id+1),($end_id-$start_id));
			}
			if($parameters['Nos']=="0"){
				//echo $parameters['id']."\r\n";
				...
				$start_id=array_search($parameters['id'],$new_all_element_ids);
				$end_array=array();
				foreach($new_all_element_ids as $naei){
					if (strpos($naei, $temp1."_".substr($only_rc,0,1)) === 0) {
						if(!in_array($naei,$end_array)){
							array_push($end_array,$naei);
						}
					}
				}
				$end_id=array_search(end($end_array),$new_all_element_ids);	
				$ids=array_slice($new_all_element_ids,((int) $start_id+1),((int) $end_id-(int) $start_id));

				/*$temp_new_all_element_ids=implode(",",$new_all_element_ids);
				$temp_new_all_element_ids=explode($parameters['id'],$temp_new_all_element_ids);
				unset($temp_new_all_element_ids[0]);
				$temp_new_all_element_ids=$temp_new_all_element_ids[1];
				$temp_new_all_element_ids=explode(",",$temp_new_all_element_ids);
				unset($temp_new_all_element_ids[0]);
				$temp_new_all_element_ids=implode(",",$temp_new_all_element_ids);
				$temp_new_all_element_ids=explode(",",$temp_new_all_element_ids);*/
				//print_r($temp_new_all_element_ids);
				//die;
				//$ids=$temp_new_all_element_ids;
			}
			if($parameters['Nos']=="-1"){
		...
		if($function_name=="doRowHide" || $function_name=="doRowShow"){
			...
			if(substr($only_rc,0,1)=="0"){
				...
				if($parameters['Nos']=="-1"){
					...
					$only_rc=((int) $only_rc+ (int)$parameters['Nos']);					
				}
			...
			$ids=array();
			foreach($new_all_element_ids as $aeil){
				if(strpos($aeil,$temp1."_".$only_rc)>-1){
					array_push($ids,$aeil);
				}
			}
			$only_rc_arr=str_split($only_rc);
			$last_new_all_element_ids1=array();
			$last_new_all_element_ids2=array();
			for($i=0;$i<sizeof($only_rc_arr)-1;$i++){
				foreach($new_all_element_ids as $naeil){
					$temp_naeil=str_replace($temp1."_","",$naeil);
					$temp_naeil=str_replace($temp1dd."_","",$temp_naeil);
					$temp_naeil=str_replace($temp1mm."_","",$temp_naeil);
					$temp_naeil=str_replace($temp1yyyy."_","",$temp_naeil);
					$temp_naeil=str_replace($temp1h."_","",$temp_naeil);
					$temp_naeil=str_replace($temp1m."_","",$temp_naeil);
					$exist=(substr($temp_naeil, 0, strlen($only_rc_arr[$i])) === $only_rc_arr[$i]);
					if($exist!=''){
						array_push($last_new_all_element_ids1,$naeil);
					}else{
						array_push($last_new_all_element_ids2,$naeil);
					}
				}
			}
			if($parameters['Nos']=="0"){
				$ids=$last_new_all_element_ids2;
			}
			if($parameters['Nos']=="-1"){
				$ids=$last_new_all_element_ids1;
			}
			$parameters['ids']=$ids;
		}	
		//echo $function_name."jony\r\n";
		return $parameters;
	}	

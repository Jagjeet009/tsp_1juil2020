...
*/
	...
	public function question_search(){
		$question_arr=array();
		$questions=$this->Survey_model->list_questions_all();
	...
	public function chart($survey_id){
		...
		//print_r($data);
			$this->load->view('analytics_chart_'.$chartType,$data);
	}	
	...
	public function analytics_png(){
		if (file_exists('uploads/chart.png')) unlink('uploads/chart.png');		//delete if exists
		$data = $this->input->post('data_url');
		list($type, $data) = explode(';', $data);
		list(, $data)      = explode(',', $data);
		$data = base64_decode($data);
		file_put_contents('uploads/chart.png', $data);	
	}
	public function analytics_png2($name){
		if (file_exists('uploads/'.$name.'.png')) unlink('uploads/'.$name.'.png');		//delete if exists
		$data = $this->input->post('data_url');
		list($type, $data) = explode(';', $data);
		list(, $data)      = explode(',', $data);
		$data = base64_decode($data);
